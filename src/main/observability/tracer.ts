// A small, plain-TS span recorder modeled on T3 Code's
// `LocalFileTracer` / `TraceSink` pair (see telemetry-error-tracking.md
// §"What to copy from T3 Code verbatim"). Orca does not use Effect, so we
...
// same NDJSON record shape the T3 sink produces (`type: 'effect-span'`,
...
export function startSpan(
  ...
  const finalize = (exit: SpanExit): void => {
    ...
    // wire-compatible with T3 Code's tracer output. Effect-only consumers
    // (the LGTM stack, jq cookbooks) can read either source unchanged.
