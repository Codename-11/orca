...
describe('terminal.multiplex pending-escape-tail threading (#7329)', () => {
  it('carries the daemon pendingEscapeTailAnsi through the SnapshotStart frame', async () => {
    ...
    try {
      ...
      // output-batcher flush timer) to completion. The frame handler is invoked
      // fire-and-forget by the multiplex control stream, so wait on the binary
      // frame side effect rather than only draining timers.
      await vi.waitFor(() => {
        expect(
          binaryFrames.some(
            (frame) =>
              decodeTerminalStreamFrame(frame)?.opcode === TerminalStreamOpcode.SnapshotStart
          )
        ).toBe(true)
      })
