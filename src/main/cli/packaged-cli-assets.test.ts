...
describe('packaged CLI assets', () => {
  ...
  it('lets the Windows launcher use the packaged executable name generated by electron-builder', async () => {
    const launcher = await readFile(windowsLauncherAsset, 'utf8')

    expect(launcher).toContain('orca-electron-executable.txt')
    expect(launcher).toContain('set /p ELECTRON_EXE=<"%ELECTRON_NAME_FILE%"')
    expect(launcher).toContain('set "ELECTRON=%APP_DIR%\\%ELECTRON_EXE%"')
