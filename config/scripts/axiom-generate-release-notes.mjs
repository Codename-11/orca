...
async function fetchUpstreamRelease(upstreamRepo, tag) {
  const res = await fetch(`https://api.github.com/repos/${upstreamRepo}/releases/tags/${encodeURIComponent(tag)}`, {
...
async function main() {
  ...
  if (!tag) {
    throw new Error('Usage: node config/scripts/axiom-generate-release-notes.mjs <tag> [--output file]')
  }
  ...
  const upstreamUrl = upstreamRelease?.html_url ?? `https://github.com/${upstreamRepo}/releases/tag/${tag}`
