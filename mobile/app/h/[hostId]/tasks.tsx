...
import { styles } from '../../../src/tasks/mobile-task-route-styles'
...
export default function MobileTasksScreen() {
  ...
  const loadForgeContext = useCallback(async (): Promise<void> => {
    if (!client || connState !== 'connected' || !tasksSupported || !forgeConnected) {
      return
    }
  ...
  useEffect(() => {
    if (!taskStateHydrated || provider !== 'forge' || !forgeConnected) {
      return
    }
